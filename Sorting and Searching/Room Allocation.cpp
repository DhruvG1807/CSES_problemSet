...
void solve(){
    ...
    vector<pair<pi, int> > customers(n);
    fo(i,0,n){
        cin>>customers[i].ff.ff>>customers[i].ff.ss;
        customers[i].ss = i;
    }
    sort(all(customers));
    ...
    fo(i, 0, n){
        int arr = customers[i].ff.ff;
        int dep = customers[i].ff.ss;
        int idx = customers[i].ss;
        // cout<<arr<<nline<<dep<<nline<<idx<<nline<<nline;
 
        if (pq.empty() || pq.top().ff >= arr){
            ans[idx] = ++room;
            pq.push(mp(dep, room));
        }
        else{
            int roomno = pq.top().ss;
            pq.pop();
            ans[idx] = roomno;
            pq.push(mp(dep, roomno));
        }
    }
